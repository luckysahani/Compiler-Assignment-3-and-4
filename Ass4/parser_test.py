...
def p_TypeDeclaration(p):
	...
	# print ST.curr_scope + ' sddsfsd '
	ST.mainsymbtbl['Main']['Classes'][p[1]['ClassName']] = ST.mainsymbtbl['Main.' + p[1]['ClassName']]['offset']
	ST.Change_scope()
...
def p_MethodDeclarator(p):
	...
	if(len(p) == 5) :
		...
		ST.Add_identifier('this','int',-1)
		for param in p[3]:
			if(not '.' in  param['Type']):
				ST.Add_identifier(param['Name'],param['Type'],-1)
			else :
				ST.Add_identifier(param['Name'],param['Type'],[['sdf_sdf'],1])
	else :
	...
	ST.mainsymbtbl[ST.curr_funcname]['paramsize'] = paramsize
...
def p_Bl(p):
	''' Bl : LCURPAREN '''
	ST.Add_scope(ST.Gen_Temp2(),'block',None)
...
def p_LocalVariableDeclarationStatement(p):
	...
	if(type(p[1]) is dict and ST.mainsymbtbl['Main']['Classes'].has_key(p[1]['Name'])) :
		for identifier in p[2]['Names'] :
			size = ST.Add_idclass(identifier,p[1]['Name'])
		# print "I am here"
	else :
		for identifier in p[2]['Names']:
...
def p_PrimaryExpression(p):
	...
	if('.' in p[1]['Name']) :
		...
		# print temp
		if(len(temp) == 2):
			if(ST.Exists_classid(temp[0]) and ST.mainsymbtbl['Main.' + ST.Get_attrclassid(temp[0],'Type')]['identifiers'].has_key(temp[1])) :
				tempcnt = 0;
				for elem in ST.mainsymbtbl['Main.' + ST.Get_attrclassid(temp[0],'Type')]['identifiers'] :
					if(elem != temp[1]):
...
def p_MethodCall(p):
	...
	p[0]['Name'] = ST.Gen_Temp()
	# print p[1]['Name']
	if(p[1]['Name'] == 'System.out.println') :
		if(len(p) == 5) :
			p[0]['Type'] = 'NULL'
			TAC.emit('Print',p[3][0]['Name'],p[3][0]['Type'],'F_CALL')
			return
		else : 
			print ("No argument to print statement")
			exit(0)
	if(p[1]['Name'] == 'System.out.get') :
		if(len(p) == 5) :
			p[0]['Type'] = 'NULL'
			TAC.emit('Get',p[3][0]['Name'],p[3][0]['Type'],'F_CALL')
			return
		else : 
			print ("No argument to get statement")
			exit(0)
	# elif(p[0]['Name'] == 'System.in.')
		...
		if(ST.mainsymbtbl[ST.curr_class]['Functions'].has_key(p[1]['Name'])) :
			...
			if(ST.Exists_classid(temp[0]) and ST.mainsymbtbl['Main.' + ST.Get_attrclassid(temp[0],'Type')]['Functions'].has_key(temp[1])) :
				...
				if(len(p) == 4):
					...
					TAC.emit(p[0]['Name'],'','Main.'+ST.Get_attrclassid(temp[0],'Type')+'.'+temp[1],'F_CALLC')
		...
		if(len(p) == 5) :
			...
			for param in p[3]:
				...
				if(param['Type'] != ST.mainsymbtbl[ST.curr_class + '.' +p[1]['Name']]['Parameters'][numele]['Type']):
	...
	if(len(p) == 2) :
		...
		if(p[1].has_key('AType')) :
			if(p[1]['Type'] == p[3]['Type']):
				...
				exit(0)
		elif( p[1].has_key('Cname') and (not ST.Exists_class(p[1]['Cname'])) and ST.mainsymbtbl[ST.curr_class]['identifiers'].has_key(p[1]['Cname'])) :
...
def parserFile(filename):
	...
	parser.parse(open(filename).read(),debug = log)
	return ST, TAC